...
class Net(nn.Module):
    ...
    def forward(self, v, q, q_len):
        ...
        v, at_prob = apply_attention(v, a)

        combined = torch.cat([v, q], dim=1)
        answer = self.classifier(combined)
        return answer, at_prob
...
class Uncertainty(nn.Sequential):
...
class TextProcessor(nn.Module):
    ...
    def forward(self, q, q_len):
        embedded = self.embedding(q.type(torch.cuda.LongTensor))
